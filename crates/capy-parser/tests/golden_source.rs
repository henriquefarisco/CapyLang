//! Golden tests for the CapyLang top-level parser (slice S2.1).
//!
//! Mirrors `tests/golden.rs` but exercises [`capy_parser::parse_source`]
//! instead of `parse_expr`. Each `.cl` fixture under
//! `tests/fixtures/source/` must have a paired `.ast` file containing the
//! canonical [`capy_ast::dump_source`] output followed by an optional
//! diagnostics tail (same format as the expression goldens).
//!
//! ## Updating goldens
//!
...
//!
//! ```text
//! CAPY_GOLDEN_UPDATE=1 cargo test -p capy-parser --test golden_source
//! ```
...
use capy_parser::{parse_source, ParseDiagnostic, ParseErrorKind};
...
fn golden() {
    ...
    for input_path in collect_fixtures() {
        ...
        let mut actual = dump_source(&result.source);
